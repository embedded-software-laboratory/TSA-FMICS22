#ifndef AHORN_SHADOW_TEST_CASE_H
#define AHORN_SHADOW_TEST_CASE_H
...
namespace se::shadow {
    ...
    public:
        ...
        void setInitialConcreteStateValuation(const std::string &flattened_name, const z3::expr &valuation);

        const std::map<unsigned int, std::map<std::string, z3::expr>> &getCycleToConcreteInputValuations() const;

        void setCycleToConcreteInputValuation(unsigned int cycle, const std::string &flattened_name,
        ...
        getExecutionHistory() const;

        void setExecutionHistory(
                std::map<unsigned int, std::vector<std::pair<std::pair<std::string, std::reference_wrapper<const Cfg>>,
                                                             std::vector<unsigned int>>>>
                        execution_history);

        const std::map<unsigned int, std::map<std::string, z3::expr>> &getCycleToConcreteStateValuations() const;

        void setCycleToConcreteStateValuations(
    ...
    private:
        ...
        boost::optional<
                std::map<unsigned int, std::vector<std::pair<std::pair<std::string, std::reference_wrapper<const Cfg>>,
                                                             std::vector<unsigned int>>>>>
                _execution_history;
        boost::optional<std::map<unsigned int, std::map<std::string, z3::expr>>> _cycle_to_concrete_state_valuations;
    };
}// namespace se::shadow

#endif//AHORN_SHADOW_TEST_CASE_H
