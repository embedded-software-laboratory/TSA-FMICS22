...
#include "se/experimental/heuristic/breadth_first_heuristic.h"
#include "se/experimental/heuristic/depth_first_heuristic.h"
...
Explorer::Explorer(const Configuration &configuration)
    : _configuration(configuration), _contexts(std::vector<std::unique_ptr<Context>>()), _statement_coverage(0),
...
std::ostream &Explorer::print(std::ostream &os) const {
    ...
    str << "(\n";
    str << "\texploration strategy: ";
    switch (_configuration.getExplorationHeuristic()) {
        case Configuration::ExplorationHeuristic::DEPTH_FIRST: {
            str << "depth-first";
            break;
        }
        case Configuration::ExplorationHeuristic::BREADTH_FIRST: {
            str << "breadth-first";
            break;
        }
        default:
            throw std::runtime_error("Invalid exploration strategy configured.");
    }
    str << ",\n";
    str << "\tcontext queue: {";
...
std::pair<bool, bool> Explorer::updateCoverage(unsigned int label, const Context &context) {
    ...
    for (const auto &incoming_edge : cfg.getIncomingEdges(label)) {
        if (incoming_edge->getType() == Edge::Type::TRUE_BRANCH) {
            _covered_branches.at(incoming_edge->getSourceLabel()).first = true;
        } else if (incoming_edge->getType() == Edge::Type::FALSE_BRANCH) {
            _covered_branches.at(incoming_edge->getSourceLabel()).second = true;
        } else {
            // do nothing
        }
    }
