#include "se/shadow/execution/evaluator.h"
...
Evaluator::Evaluator(Solver &solver) : _solver(&solver), _context(nullptr), _expression_stack(std::stack<z3::expr>()) {}
...
void Evaluator::visit(const ir::ChangeExpression &expression) {
    // XXX SSE executes the new version during BSE (= Phase 2)
