...
TestSuite::TestSuite(Solver &solver) : _solver(&solver), _test_cases(std::vector<std::shared_ptr<TestCase>>()) {}

const std::vector<std::shared_ptr<TestCase>> &TestSuite::getTestCases() const {
    return _test_cases;
}

std::shared_ptr<TestCase> TestSuite::deriveTestCase(const Context &context) {
    ...
    std::shared_ptr<TestCase> test_case =
            std::make_shared<TestCase>(initial_concrete_state_valuations, cycle_to_concrete_input_valuations);
