...
Evaluator::Evaluator(ShadowProcessingMode shadow_processing_mode, shadow::Solver &solver)
    : _shadow_processing_mode(shadow_processing_mode), _solver(&solver), _context(nullptr) {}
...
void Evaluator::visit(const ir::ChangeExpression &expression) {
    switch (_shadow_processing_mode) {
        case ShadowProcessingMode::NONE:
            throw std::runtime_error("ChangeExpression encountered while shadow processing mode is set to none!");
