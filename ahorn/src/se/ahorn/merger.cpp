...
std::unique_ptr<Context> Merger::merge(std::unique_ptr<Context> context_1, std::unique_ptr<Context> context_2) {
    ...
    for (auto it = main_cfg.flattenedInterfaceBegin(); it != main_cfg.flattenedInterfaceEnd(); ++it) {
        ...
        unsigned int highest_symbolic_version_1 = state_1.getHighestVersion(flattened_name, cycle);
        assert(highest_symbolic_version_1 == state_1.getHighestVersion(flattened_name, cycle, true));
        unsigned int highest_symbolic_version_2 = state_2.getHighestVersion(flattened_name, cycle);
        assert(highest_symbolic_version_2 == state_2.getHighestVersion(flattened_name, cycle, true));
