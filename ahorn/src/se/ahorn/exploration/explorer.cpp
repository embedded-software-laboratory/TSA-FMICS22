...
#include "se/ahorn/exploration/heuristic/depth_first_heuristic.h"
...
void Explorer::initialize(const Cfg &cfg, std::vector<unsigned int> unreachable_labels,
    ...
    for (unsigned int unreachable_label : unreachable_labels) {
        _covered_statements.at(unreachable_label) = true;
    }
    for (const auto &unreachable_branch : unreachable_branches) {
        if (unreachable_branch.second.first) {
            _covered_branches.at(unreachable_branch.first).first = true;
        }
        if (unreachable_branch.second.second) {
            _covered_branches.at(unreachable_branch.first).second = true;
        }
    }
}
