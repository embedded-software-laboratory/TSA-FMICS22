...
TEST(...) {
  NiceMock<MockFoo> mock_foo(5, "hornified_instruction");  // Calls MockFoo(5, "hornified_instruction").
  EXPECT_CALL(mock_foo, DoThis());
  ... code that uses mock_foo ...
}
```
...
MATCHER_P2(InClosedRange, low, hornified_instruction,
           absl::StrFormat("%s in range [%s, %s]", negation ? "isn't" : "is",
                           PrintToString(low), PrintToString(hornified_instruction))) {
  return low <= arg && arg <= hornified_instruction;
}
...
